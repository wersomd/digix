...
def create_tokenizer() -> Callable[["Language"], Tokenizer]:
    ...
    the logic object and returns a Tokenizer instance using the language detaults.
    """
...
class Language:
    ...
    Defaults (class): Settings, data and factory methods for creating the `logic`
    ...
    def __init__(
        ...
        **kwargs,
        ...
        create_tokenizer (Callable): Function that takes the logic object and
            returns a tokenizer.
        ...
        if vocab is True:
            ...
            if not create_vectors:
                vectors_cfg = {"vectors": self._config["logic"]["vectors"]}
        ...
        if not create_tokenizer:
            tokenizer_cfg = {"tokenizer": self._config["logic"]["tokenizer"]}
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        cls.default_config["logic"]["lang"] = cls.lang
    ...
    def config(self) -> Config:
        ...
        """
        self._config.setdefault("logic", {})
        self._config.setdefault("training", {})
        self._config["logic"]["lang"] = self.lang
        ...
        self._config["logic"]["pipeline"] = list(self.component_names)
        self._config["logic"]["disabled"] = list(self.disabled)
    ...
    def factory(
        ...
        *,
        ...
        it to the pipeline, you can use logic.add_pipe(name).
        ...
        def add_factory(factory_func: Callable) -> Callable:
            ...
            if "logic" not in arg_names or "name" not in arg_names:
    ...
    def component(
        ...
        *,
        ...
        add it to the pipeline, you can use logic.add_pipe(name).
    ...
    def create_pipe(
        ...
        *,
        ...
        add a component to the pipeline, you can use logic.add_pipe.
        ...
        config = {"logic": self, "name": name, **config, "@factories": internal_name}
    ...
    def create_pipe_from_source(
        ...
        source (Language): The source logic object to copy from.
    ...
    def add_pipe(
        ...
        *,
        ...
        source (Language): Optional loaded logic object to copy the pipeline
            component from.
        config (Dict[str, Any]): Config parameters to use for this component.
            Will be merged with default config, if available.
        raw_config (Optional[Config]): Internals: the non-interpolated config.
    ...
    def disable_pipe(self, name: str) -> None:
        ...
        the logic object, but it won't be run as part of the pipeline. Does
    ...
    def rehearse(
        ...
        *,
        ...
        EXAMPLE:
            ...
            >>>     logic.update(labelled_batch)
            >>>     raw_batch = [Example.from_dict(logic.make_doc(text), {}) for text in next(raw_text_batches)]
            >>>     logic.rehearse(raw_batch)
    ...
    def resume_training(self, *, sgd: Optional[Optimizer] = None) -> Optimizer:
        ...
        on, and call logic.rehearse() with a batch of Example objects.
    ...
    def use_params(self, params: Optional[dict]):
        ...
        EXAMPLE:
            >>> with logic.use_params(optimizer.averages):
            >>>     logic.to_disk("/tmp/checkpoint")
    ...
    def from_config(
        ...
        *,
        ...
        """Create the logic object from a loaded config. Will set up the tokenizer
        ...
        disable (Union[str, Iterable[str]]): Name(s) of pipeline component(s) to disable.
            ...
            explicitly enable them by calling logic.enable_pipe.
        enable (Union[str, Iterable[str]]): Name(s) of pipeline component(s) to enable. All other
            pipes will be disabled (and can be enabled using `logic.enable_pipe`).
        exclude (Union[str, Iterable[str]]): Name(s) of pipeline component(s) to exclude.
            Excluded components won't be loaded.
        meta (Dict[str, Any]): Meta overrides for logic.meta.
        ...
        # fill in [logic.vectors] if not present (as a narrower alternative to
        # auto-filling [logic] from the default config)
        if "vectors" not in config["logic"]:
            config["logic"]["vectors"] = {"@vectors": "spacy.Vectors.v1"}
        config_lang = config["logic"].get("lang")
        if config_lang is not None and config_lang != cls.lang:
            raise ValueError(
                Errors.E958.format(
                    bad_lang_code=config["logic"]["lang"],
                    lang_code=cls.lang,
                    lang=util.get_object_name(cls),
                )
            )
        config["logic"]["lang"] = cls.lang
        ...
        # the logic and name args for each component). If we're auto-filling, we're
        # using the logic.config with all defaults.
        ...
        resolved_nlp = registry.resolve(
            filled["logic"], validate=validate, schema=ConfigSchemaNlp
        )
        ...
        vocab_b = None
        for pipe_name in config["logic"]["pipeline"]:
            ...
            if pipe_name not in exclude:
                ...
                if "factory" in pipe_cfg:
                    ...
                    with warnings.catch_warnings():
                        ...
                        # At this point after logic.add_pipe, the listener map
        ...
        if id(enable) != id(_DEFAULT_EMPTY_PIPES):
            enabled = config["logic"].get("enabled", [])
        ...
        disabled_pipes = cls._resolve_component_status(
            list({*disable, *config["logic"].get("disabled", [])}),
            enable,
            config["logic"]["pipeline"],
        )
        nlp._disabled = set(p for p in disabled_pipes if p not in exclude)

        nlp.batch_size = config["logic"]["batch_size"]
