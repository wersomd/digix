...
class Warnings(metaclass=ErrorsWithCodes):
    ...
    W012 = ("A Doc object you're adding to the PhraseMatcher for pattern "
            ...
            "try using `logic.make_doc(text)` or process all texts as a stream "
            "using `list(logic.tokenizer.pipe(all_texts))`.")
    ...
    W030 = ("Some entities could not be aligned in the text \"{text}\" with "
            ...
            "`spacy.training.offsets_to_biluo_tags(logic.make_doc(text), entities)`"
    ...
    W089 = ("As of spaCy v3.0, the `logic.begin_training` method has been renamed "
            "to `logic.initialize`.")
    ...
    W096 = ("The method `logic.disable_pipes` is now deprecated - use "
            "`logic.select_pipes` instead.")
    ...
    W105 = ("As of spaCy v3.0, the `{matcher}.pipe` method is deprecated. If you "
            "need to match on a stream of documents, you can use `logic.pipe` and "
    ...
    W118 = ("Term '{term}' not found in glossary. It may however be explained in documentation "
            ...
            "`logic.meta[\"sources\"]` for any relevant links.")
...
class Errors(metaclass=ErrorsWithCodes):
    ...
    E002 = ("Can't find factory for '{name}' for language {lang} ({lang_code}). "
            "This usually happens when spaCy calls `logic.{method}` with a custom "
    ...
    E008 = ("Can't restore disabled pipeline component '{name}' because it "
            ...
            "`logic.select_pipes` or after restoring the disabled components.")
    ...
    E030 = ("Sentence boundaries unset. You can add the 'sentencizer' "
            "component to the pipeline with: `logic.add_pipe('sentencizer')`. "
    ...
    E062 = ("Cannot find empty bit for new lexical flag. All bits between 0 "
            ...
            "`logic.vocab.add_flag(your_func, flag_id=IS_ALPHA`.")
    ...
    E088 = ("Text of length {length} exceeds maximum of {max_length}. The "
            ...
            "`logic.max_length` limit. The limit is in number of characters, so "
    ...
    E150 = ("The language of the `logic` object and the `vocab` should be the "
            "same, but found '{logic}' and '{vocab}' respectively.")
    ...
    E155 = ("The pipeline needs to include a {pipe} in order to use "
            ...
            "Try using `logic()` instead of `logic.make_doc()` or `list(logic.pipe())` "
            "instead of `list(logic.tokenizer.pipe())`.")
    ...
    E870 = ("Could not serialize the DocBin because it is too large. Consider "
            ...
            "the 'path'.")
    E871 = ("Error encountered in logic.pipe with multiprocessing:\n\n{error}")
    ...
    E888 = ("Can't replace listeners for '{name}' ({pipe}): invalid upstream "
            ...
            "or Transformer component. If you didn't call logic.replace_listeners "
    ...
    E889 = ("Can't replace '{tok2vec}' listeners of component '{name}' because "
            ...
            "If you didn't call logic.replace_listeners manually, this is likely "
            "a bug in spaCy.")
    ...
    E899 = ("Can't serialize trainable pipe '{name}': the `vocab` attribute "
            "is not set or None. If you've implemented a custom component, make "
            "sure to store the current `logic` object's vocab as `self.vocab` in "
    ...
    E914 = ("Executing {name} callback failed. Expected the function to "
            "return the logic object but got: {value}. Maybe you forgot to return "
    ...
    E926 = ("It looks like you're trying to modify `logic.{attr}` directly. This "
            "doesn't work because it's an immutable computed property. If you "
            "need to modify the pipeline, use the built-in methods like "
            "`logic.add_pipe`, `logic.remove_pipe`, `logic.disable_pipe` or "
            "`logic.enable_pipe` instead.")
    ...
    E943 = ("Executing `before_creation` callback failed. Expected the function to "
            ...
            "returned the initialized logic object instead?")
    ...
    E945 = ("Can't copy pipeline component '{name}' from source. Expected "
            "loaded logic object, but got: {source}")
    ...
    E958 = ("Language code defined in config ({bad_lang_code}) does not match "
            ...
            "If you want to create an logic object from a config, make sure to "
    ...
    E964 = ("The pipeline component factory for '{name}' needs to have the "
            ...
            "receives the current logic object and lets you access the vocab\n- "
    ...
    E966 = ("`logic.add_pipe` now takes the string name of the registered component "
            "factory, not a callable component. Expected string, but got "
            "{component} (name: '{name}').\n\n- If you created your component "
            "with `logic.create_pipe('name')`: remove logic.create_pipe and call "
            "`logic.add_pipe('name')` instead.\n\n- If you passed in a component "
            "like `TextCategorizer()`: call `logic.add_pipe` with the string name "
            "instead, e.g. `logic.add_pipe('textcat')`.\n\n- If you're using a custom "
            ...
            "`logic.add_pipe('your_name')` to add it to the pipeline.")
    E967 = ("No {meta} meta information found for '{name}'. This is likely a bug in spaCy.")
    E968 = ("`logic.replace_pipe` now takes the string name of the registered component "
            "factory, not a callable component. Expected string, but got "
            "{component}.\n\n- If you created your component with"
            "with `logic.create_pipe('name')`: remove `logic.create_pipe` and call "
            "`logic.replace_pipe('{name}', 'name')` instead.\n\n- If you passed in a "
            "component like `TextCategorizer()`: call `logic.replace_pipe` with the "
            "string name instead, e.g. `logic.replace_pipe('{name}', 'textcat')`.\n\n"
            ...
            "then run `logic.replace_pipe('{name}', 'your_name')`.")
    ...
    E985 = ("Can't load model from config file: no [logic] section found.\n\n{config}")
    E986 = ("Could not create any training batches: check your input. "
            "Are the train and dev paths defined? Is `discard_oversize` set appropriately? ")
    E989 = ("`logic.update()` was called with two positional arguments. This "
    ...
    E991 = ("The function `logic.select_pipes` should be called with either a "
    ...
    E992 = ("The function `select_pipes` was called with `enable`={enable} "
            ...
            "for the `logic` pipeline with components {names}.")
    E993 = ("The config for the logic object needs to include a key `lang` specifying "
    ...
    E1000 = ("The Chinese word segmenter is pkuseg but no pkuseg model was "
             ...
             'logic.tokenizer.initialize(pkuseg_model="default")')
    ...
    E1004 = ("Missing lemmatizer table(s) found for lemmatizer mode '{mode}'. "
             ...
             "call `logic.initialize()` to load in the data?")
