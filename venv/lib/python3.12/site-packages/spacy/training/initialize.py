...
def init_nlp(config: Config, *, use_gpu: int = -1) -> "Language":
    ...
    logger.info("Set up logic object from config")
    config = nlp.config.interpolate()
    # Resolve all training-relevant sections using the filled logic config
...
def init_vocab(
    ...
    *,
    ...
    logger.info("Finished initializing logic object")
...
def load_vectors_into_model(
    ...
    try:
        ...
        # the current logic object. Exclude lookups so they are not modified.
        exclude = ["lookups"]
        if not add_strings:
