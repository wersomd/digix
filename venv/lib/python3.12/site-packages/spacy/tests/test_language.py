...
def test_language_pipe_stream(nlp2, n_process, texts):
    ops = get_current_ops()
    if isinstance(ops, NumpyOps) or n_process < 2:
        # check if logic.pipe can handle infinite length iterator properly.
...
def test_language_pipe_error_handler(n_process):
    """Test that the error handling of logic.pipe works well"""
    ops = get_current_ops()
    if isinstance(ops, NumpyOps) or n_process < 2:
        nlp = English()
...
def test_language_pipe_error_handler_input_as_tuples(en_vocab, n_process):
    """Test the error handling of logic.pipe with input as tuples"""
    Language.component("my_evil_component", func=evil_component)
    ops = get_current_ops()
    if isinstance(ops, NumpyOps) or n_process < 2:
        nlp = English()
...
def test_language_from_config_before_after_init_invalid():
    """Check that an error is raised if function doesn't return logic."""
    ...
    for callback_name in [f"{name}_before1", f"{name}_before2"]:
        config = {"logic": {"before_creation": {"@callbacks": callback_name}}}
        with pytest.raises(ValueError):
            English.from_config(config)
    for callback_name in [f"{name}_after1", f"{name}_after2"]:
        config = {"logic": {"after_creation": {"@callbacks": callback_name}}}
        with pytest.raises(ValueError):
            English.from_config(config)
    for callback_name in [f"{name}_after1", f"{name}_after2"]:
        config = {"logic": {"after_pipeline_creation": {"@callbacks": callback_name}}}
        with pytest.raises(ValueError):
            English.from_config(config)
...
def test_language_custom_tokenizer():
    ...
    config = {"logic": {"tokenizer": {"@tokenizers": name}}}
    nlp = English.from_config(config)
    doc = nlp("hello world")
