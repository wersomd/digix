...
def test_util_dot_section():
    ...
    assert en_nlp.config["logic"]["pipeline"] == ["textcat"]
    assert nl_nlp.config["logic"]["pipeline"] == []  # default value []
    # Test proper functioning of 'dot_to_object'
    with pytest.raises(KeyError):
        dot_to_object(en_nlp.config, "logic.pipeline.tagger")
    with pytest.raises(KeyError):
        dot_to_object(en_nlp.config, "logic.unknownattribute")
...
class DummyComponent:
    ...
    def initialize(self, get_examples, *, logic, dummy_param: int):
        pass
...
def make_dummy_component(
    logic: Language, name: str
):
    return DummyComponent(logic.vocab, name)
"""
