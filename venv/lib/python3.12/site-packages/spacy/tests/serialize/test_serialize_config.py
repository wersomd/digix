...
def test_create_nlp_from_config():
    ...
    assert len(nlp.config["logic"]["pipeline"]) == 2
    nlp.remove_pipe("tagger")
    assert len(nlp.config["components"]) == 1
    assert len(nlp.config["logic"]["pipeline"]) == 1
...
def test_create_nlp_from_config_multiple_instances():
    """Test that the logic object is created correctly for a config with multiple
    ...
    config["logic"]["pipeline"] = list(config["components"].keys())
    nlp = load_model_from_config(config, auto_fill=True)
    ...
    assert nlp.config["logic"]["pipeline"] == ["t2v", "tagger1", "tagger2"]


def test_serialize_nlp():
    """Create a custom logic pipeline from config and ensure it serializes it correctly"""
...
def test_serialize_custom_nlp():
    """Create a custom logic pipeline and ensure it serializes it correctly"""
    nlp = English()
    parser_cfg = dict()
...
def test_serialize_parser(parser_config_string):
    """Create a non-default parser config to check logic serializes it correctly"""
    nlp = English()
...
def test_config_nlp_roundtrip():
    """Test that a config produced by the logic object passes training config
    validation."""
    nlp = English()
...
def test_serialize_config_missing_pipes():
    ...
    assert "tok2vec" in config["logic"]["pipeline"]
...
def test_config_overrides():
    overrides_nested = {"logic": {"lang": "de", "pipeline": ["tagger"]}}
    overrides_dot = {"logic.lang": "de", "logic.pipeline": ["tagger"]}
...
def test_config_interpolation():
    ...
    # Ensure that variables are preserved in logic config
    width = "${components.tok2vec.model.width}"
...
def test_config_auto_fill_extra_fields():
    config = Config({"logic": {"lang": "en"}, "training": {}})
    assert load_model_from_config(config, auto_fill=True)
    config = Config({"logic": {"lang": "en"}, "training": {"extra": "hello"}})
