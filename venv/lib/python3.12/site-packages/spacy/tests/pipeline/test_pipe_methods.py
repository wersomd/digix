...
def test_issue3880():
    """Test that `logic.pipe()` works when an empty string ends the batch.
...
def test_disable_enable_pipes():
    ...
    assert nlp.config["logic"]["disabled"] == [f"{name}1"]
    nlp("hello")
    ...
    assert nlp.disabled == []
    assert nlp.pipe_names == [f"{name}1", f"{name}2"]
    assert nlp.config["logic"]["disabled"] == []
    nlp("world")
    ...
    assert nlp.config["logic"]["disabled"] == [name]
    nlp("?")
    assert results[f"{name}1"] == "!"
...
def test_enable_disable_conflict_with_config():
    """Test conflict between enable/disable w.r.t. `logic.disabled` set in the config."""
    nlp = English()
    nlp.add_pipe("tagger")
    nlp.add_pipe("senter")
    nlp.add_pipe("sentencizer")
    ...
    with make_tempdir() as tmp_dir:
        ...
        assert spacy.load(
            tmp_dir, enable=["tagger"], config={"logic": {"disabled": ["senter"]}}
        ...
        with pytest.raises(ValueError):
            spacy.load(
                tmp_dir,
                enable=["senter"],
                config={"logic": {"disabled": ["senter", "tagger"]}},
            )
