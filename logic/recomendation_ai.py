...
client = OpenAI(api_key=os.getenv('DIGIX_AI_KEY'))
...
async def get_relevance_score(client_query: str, specialization: str, full_name: str, city: str, age: int, gender: str,
    ...
    try:
        ...
        print(f'Relevance_score: {full_name} : {relevance_score}, {cv_text}')
...
async def compare_texts(client_query: str, specialists: List[Tuple[int, str, str, str, int, str, str]]) -> List[int]:
    ...
    relevance_scores = [(spec_id, score) for spec_id, score in relevance_scores if score > 0]
